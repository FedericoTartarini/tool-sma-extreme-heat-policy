...
class TestHomePage:
    ...
    def test_visibility_text(self, page: Page):
        ...
        expect(
            page.locator("#react-select-2--value div").filter(has_text="Soccer")
        ).to_be_visible()
        expect(page.locator("#sport-image").get_by_role("img")).to_be_visible()
        expect(
            page.locator("#react-select-3--value div").filter(
                has_text="Camperdown, NSW,"
            )
        ).to_be_visible()
    ...
    def test_click_dropdown(self, page: Page):
        ...
        for sport in sports:
            page.locator("#react-select-2--value div").filter(
                has_text=drop_down_sport
            ).click()
            page.locator("#react-select-2--value").get_by_role("combobox").fill(sport)
            page.locator("#react-select-2--value").get_by_role("combobox").press(
                "Enter"
            )
            expect(page.get_by_role("img", name="Heat stress chart")).to_be_visible()
            drop_down_sport = sport

    def test_selecting_non_esistent_sport(self, page: Page):
        page.goto("/")
